...
func (*LogConsumer) HandleMessage(msg *nsq.Message) error {
	...
	if pubKey == nil {
		log.Warnf("can not find public key of client: %s", nodeId)
		return nil
	}
	...
	err = nil
	for i := 0; i < 5; i++ {
		if err = db.SaveFromClient(nodeId, batch.Timestamp, batch.ActionLog); err == nil {
			break
		}
...
func getPubKeyFromDbOrTracker(nodeIdStr string) (pk *rsa.PublicKey) {
	pubKey := db.GetClientPubKey(nodeIdStr)
	var err error
	if len(pubKey) == 0 {
		for i := 1; ; i++ {
			pubKey, err = client.ClientPubKey(nodeIdStr)
			if err != nil {
				log.Warnf("get client [%s] public key from tracker error: %v", nodeIdStr, err)
				duration := i
				if i > 30 {
					duration = 30
				}
			...
			if len(pubKey) == 0 {
				...
				db.SaveClientPubKey(nodeIdStr, pubKey)
				break
			}
		}
	}
	...
	if err != nil {
		log.Warnf("parse client [%s] public key [%x] error: %v", nodeIdStr, pubKey, err)
		return nil
	}
	return pk
}
